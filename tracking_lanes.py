...
def averaged_slope_intercept(img, lines):
    ...
    try:
        ...
        left_line = make_coordinates(img, left_fit_averaged)
        right_line = make_coordinates(img, right_fit_averaged)
        return np.array([left_line, right_line])
    except:
        print('out lane')
        cv2.putText(img,'Out Lane',(50,50), cv2.FONT_HERSHEY_SIMPLEX, 4,(255,255,255),2,cv2.LINE_AA)
        pass
...
def lines_detection(img):
    ...
    averaged_lines = averaged_slope_intercept(img, lines)
